...
class KFAC(Optimizer):
    ...
    def _precond(self, weight, bias, group, state):
        ...
        if bias is not None:
            gb = bias.grad.data
            # print(g.shape, gb.shape)
            g = torch.cat([g, gb.view(gb.shape[0], 1)], dim=1).transpose(1, 0)
            # print(g.shape)
    ...
    def _compute_covs(self, group, state):
        ...
        if sub_mod.lin.weight.ndim == 3:
            x = x.repeat(sub_mod.lin.weight.shape[0], 1)
